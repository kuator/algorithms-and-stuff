...
public class Percolation {
  ...
  private boolean isValidCell(int row, int col) {
    if (row < 1 || row > this.n || col < 1 || col > this.n) return false;
    return true;
  }
  ...
  private void connectWithOpenNeighbours(int row, int col) {
    ...
    for (int k = 0; k < neighbours.length; k++) {
      ...
      if (!isValidCell(nRow, nCol) || !isOpen(nRow, nCol)) continue;
      this.union(row, col, getCellNumber(nRow, nCol));
    }
  }

  public void open(int row, int col) {
    if (!isValidCell(row, col)) throw new IllegalArgumentException("Values are out of range");
    ...
    if (row == 1) this.union(row, col, top - 1);

    if (row == n) this.union(row, col, bottom - 1);
